// Notebook — backend module
// Stores notes in this extension's private storage as notes.json and
// answers CRUD requests from the frontend over the sendToFrontend /
// onFrontendMessage bridge.
...
interface Note {
  ...
  text: string
  tag: string | null
  source: string | null // e.g. character/chat name the snippet was pulled from
  createdAt: string
  updatedAt: string
}
...
async function loadNotes(): Promise<Note[]> {
  return spindle.storage.getJson<Note[]>(NOTES_FILE, { fallback: [] })
}
