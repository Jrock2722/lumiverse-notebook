// Notebook — frontend module
// A drawer tab + input bar shortcut for jotting down quotes, ideas, and
// message snippets without leaving the chat.
...
interface SpindleFrontendContext {
  ...
  ready?: () => void
  ui: {
    registerDrawerTab: (opts: any) => any
    registerInputBarAction: (opts: any) => any
    showModal: (opts: any) => any
    showConfirm: (opts: any) => Promise<{ confirmed: boolean }>
    showContextMenu: (opts: any) => Promise<{ selectedKey: string | null }>
  }
  toast?: {
...
const NOTEBOOK_ICON = `<svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export function setup(ctx: SpindleFrontendContext) {
  ...
  const unsubBackend = ctx.onBackendMessage((payload: any) => {
    switch (payload.type) {
      ...
      case 'note_updated': {
        ...
        if (idx !== -1) notes[idx] = payload.note
        renderList()
        break
      }
      case 'note_deleted':
  ...
  function openNoteModal(existing?: Note) {
  ...
  async function handleNoteMenu(note: Note, x: number, y: number) {
    ...
    if (selectedKey === 'copy') {
      ...
      if (confirmed) ctx.sendToBackend({ type: 'delete_note', id: note.id })
    }
  }
  ...
  function renderList() {
    ...
    if (filtered.length === 0) {
      ...
      empty.textContent = notes.length === 0
        ? 'Nothing saved yet. Hit "New Note" to add your first one.'
